...
class Retriever:
    ...
    def _embed_query(self, q):
        if self._embed_model:
            try:
                return self._embed_model.encode(q).tolist()
            except Exception:
                pass
    ...
    def retrieve(self, query, k=3):
        if not self.indexed:
            return [{"id": "sample", "text": "This is a sample document. Run scripts/build_embeddings.py and scripts/build_index.py to enable real retrieval."}]

        try:
            with open(self.emb_path, 'r', encoding='utf-8') as f:
                embeddings = json.load(f)
        except Exception:
            return [{"id": "emb_read_error", "text": "Failed to read embeddings."}]
